...
namespace MyLibrary
{
    ...
    public class MenuLib
    {
        ...
        public static string ReadOption(string[] options, string message)
        {
            ...
            if (!int.TryParse(Console.ReadLine(), out int chosenOption) || chosenOption < 0 || chosenOption > options.Length - 1)
            {
                ReadOption(options, "Opção inválida. Digite uma opção válida:");
            }
        ...
        public static double ReadMoneyValue(string message)
        {
            ...
            {
                ReadMoneyValue("Valor digitado inválido. Digite um valor válido:");
            }
            return value;
        }
